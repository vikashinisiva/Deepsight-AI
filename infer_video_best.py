...
# Load ORIGINAL baseline model (the one that works!)
...
# Original MTCNN detector
detector = MTCNN()

@torch.no_grad()
def infer_best(video_path, max_frames=30):
    """Best working inference - original model + MTCNN"""
    # Extract frames
    ...
    for fp in sorted(glob.glob(os.path.join(temp_dir, "*.jpg"))):
        ...
        # Use MTCNN (original working method)
        ...
        # Convert and predict (same as working version)
